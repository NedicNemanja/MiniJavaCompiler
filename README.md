## <a name="hw2">MiniJava Static Checking (Semantic Analysis)</a>

This project, which consists of building a compiler for MiniJava, a subset of Java. MiniJava is designed so that its programs can be compiled by a full Java compiler like javac.
...
The MiniJava grammar in JavaCC form is [here](https://github.com/NedicNemanja/MiniJavaCompiler/blob/master/minijava.jj). JTB tool was used to convert it into a grammar that produces class hierarchies. Then I wrote visitors who will take control over the MiniJava input file and will tell whether it is semantically correct, or will print an error message. It isn’t necessary for the compiler to report precisely what error it encountered and compilation can end at the first error. But you should not miss errors or report errors in correct programs.

The visitors I will built are subclasses of the visitors generated by JTB, but they may also contain methods and fields to hold information during static checking, to transfer information from one visitor to the next, etc. In the end, these is a Main class that runs the semantic analysis initiating the parser that was produced by JavaCC and executing the visitors. 

Corresponding offsets are shown in the example below:
...
You will need to execute the produced LLVM IR files in order to see that their output is the same as compiling the input java file with `javac` and executing it with `java`. To do that, you will need Clang with version >=4.0.0\. You may download it on your Linux machine.
...
That is, the program must compile to LLVM IR all .java files given as arguments. Moreover, the outputs must be stored in files named `file1.ll`, `file2.ll`, ... `fileN.ll` respectively.

## Tips

*   I used a lot of registers in order to 'glue' expressions together. This means that each visitor will produce the code for storing the value of an expression to a register, and then return the name of that register so that other expressions may use it, if necessary.
...
*   Because registers are single-assignment, I needed to keep a counter to produce new ones. For example, I produced registers of the form `%_1`, `%_2`, etc.
*   Only supported compilation to a 64-bit architecture: pointers are 8-bytes long.
...
*   Need to check each array access in order not to write or read beyond the limits of an array. If an illegal read/write is attempted, you will print the message "Out of bounds" and the program will exit (you may call the `@throw_oob` defined below for that). Of course, you need to know the length of an array for that.
*   Need to check if an array is allocated with a negative length, and do the same process as above in that case.
*   Defined the following helper methods once in your output files, in order to be able to call `@calloc`, `@print_int` and `@throw_oob`.
